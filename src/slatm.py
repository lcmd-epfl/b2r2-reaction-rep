...
def get_twobody(ncharge_a, ncharge_b, coords_a, coords_b, 
                sigma=0.05, rcut=4.8, gridspace=0.03):
    r0 = 0.1
    ...
    if distance < rcut:
        potential = 0.5 * gaussian(grid - distance, sigma=sigma) * (1 / grid ** 6) * ncharge_b * ncharge_a * gridspace
    else:
...
def get_slatm(ncharges, coords, sigma=0.05, rcut=4.8,
        gridspace=0.03, elements=[1,6,7,8,9,17]):
        ...
        for i, ncharge_a in enumerate(ncharges):
            for j in range(i):
                ...
                if bag == bag_candidate or bag == inv_bag_candidate:
                    potential = get_twobody(ncharge_a, ncharge_b, 
                                        coords[i], coords[j],
                                        sigma=sigma, rcut=rcut, 
                                        gridspace=gridspace)
